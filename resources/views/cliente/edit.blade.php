...
<div class="container-fluid px-4">
    ...
    <div class="card text-bg-light">
        <form action="{{ route('clientes.update',['cliente'=>$cliente]) }}" method="post" id="clienteForm">
            @method('PATCH')
            @csrf
            <div class="card-body">
                <div class="row g-3">
                    ...
                    <div class="col-md-6">
                        ...
                        <input type="text" class="form-control" value="{{ strtoupper($cliente->persona->tipo->value) }}" readonly>
                        <input type="hidden" name="tipo" value="{{ $cliente->persona->tipo->value }}">
                    </div>
                    ...
                    <div class="col-md-6">
                        ...
                        <input type="text" name="telefono" id="telefono" class="form-control" value="{{ old('telefono', $cliente->persona->telefono) }}" placeholder="8 dígitos (6, 7 o 2) o + hasta 12 dígitos">
                        @error('telefono')
                        <small class="text-danger">{{'*'.$message}}</small>
                        @enderror
                    ...
                    <div class="col-md-6">
                        ...
                        <select class="form-select" name="documento_id" id="documento_id">
                            ...
                            <option value="{{$item->id}}" {{ old('documento_id', $cliente->persona->documento_id) == $item->id ? 'selected' : '' }}>{{$item->nombre}}</option>
                            @endforeach
                        </select>
                        @error('documento_id')
                        <small class="text-danger">{{'*'.$message}}</small>
...
<script>
    $(document).ready(function() {
        let esCI = false;
        const tipoCliente = "{{ $cliente->persona->tipo->value }}";
        ...
        function dividirNombresNatural() {
            const razonSocial = "{{ $cliente->persona->razon_social }}";
            if (tipoCliente === 'NATURAL' && razonSocial) {
        ...
        function dividirNumeroDocumentoCI() {
            const numeroDocumentoCompleto = "{{ old('numero_documento', $cliente->persona->numero_documento) }}";
        ...
        @else
            esCI = ("{{ $cliente->persona->documento_id }}" == '1');
        @endif
