            ...
            <div class="card-body">
                <div class="row g-3">
                    ...
                    <div class="col-md-6">
                        ...
                        <input type="text" class="form-control" value="{{ strtoupper($proveedore->persona->tipo->value) }}" readonly>
                        <input type="hidden" name="tipo" value="{{ $proveedore->persona->tipo->value }}">
                    </div>
                    ...
                    <div class="col-md-6">
                        ...
                        <input type="email" name="email" id="email" class="form-control" value="{{ old('email', $proveedore->persona->email) }}">
                        @error('email')
                        <small class="text-danger">{{'*'.$message}}</small>
                        @enderror
                    </div>
                    ...
                    <div class="col-md-6">
                        ...
                        <input type="text" name="telefono" id="telefono" class="form-control" value="{{ old('telefono', $proveedore->persona->telefono) }}" placeholder="8 dígitos (6, 7 o 2) o + hasta 12 dígitos">
                        @error('telefono')
                        <small class="text-danger">{{'*'.$message}}</small>
                        @enderror
                    ...
                    <div class="col-md-6">
                        ...
                        <select class="form-select" name="documento_id" id="documento_id">
                            ...
                            <option value="{{$item->id}}" {{ old('documento_id', $proveedore->persona->documento_id) == $item->id ? 'selected' : '' }}>{{$item->nombre}}</option>
                            @endforeach
                        </select>
                        @error('documento_id')
                        <small class="text-danger">{{'*'.$message}}</small>
...
<script>
    $(document).ready(function() {
        let esCI = false;
        const tipoProveedor = "{{ $proveedore->persona->tipo->value }}";
        ...
        // Función para dividir nombres y apellidos de persona natural
        function dividirNombresNatural() {
            const razonSocial = "{{ $proveedore->persona->razon_social }}";
            if (tipoProveedor === 'NATURAL' && razonSocial) {
                const partes = razonSocial.trim().split(' ');
                if (partes.length >= 2) {
                    $('#nombres').val(partes[0]);
                    $('#apellido_paterno').val(partes[1]);
                    if (partes.length > 2) {
                        $('#apellido_materno').val(partes.slice(2).join(' '));
                    }
                }
            }
        }

        // Función para dividir número de documento y complemento para CI
        function dividirNumeroDocumentoCI() {
            const numeroDocumentoCompleto = "{{ old('numero_documento', $proveedore->persona->numero_documento) }}";
            if (esCI && numeroDocumentoCompleto) {
                // Separar por guion
                const partes = numeroDocumentoCompleto.split('-');
                if (partes.length === 2) {
                    $('#numero_documento').val(partes[0]); // Parte numérica
                    $('#complemento').val(partes[1]);      // Complemento
                } else if (partes.length === 1) {
                    // Si no hay guion, asumir que es solo el número
                    $('#numero_documento').val(partes[0]);
                    $('#complemento').val('');
                }
            }
        }

        // Mostrar campos según tipo de proveedor
        function configurarCamposPorTipo() {
            if (tipoProveedor === 'NATURAL') {
                ...
                dividirNombresNatural();
            } else if (tipoProveedor === 'JURIDICA') {
        ...
        $('#documento_id').on('change', function() {
            ...
            if (esCI) {
                ...
                // Cuando se selecciona CI, dividir el número de documento existente
                dividirNumeroDocumentoCI();
            } else {
                $('#box-complemento').hide();
                $('#box-numero-documento').removeClass('col-md-3').addClass('col-md-6');
                // Limpiar complemento cuando no es CI
                $('#complemento').val('');
            }
        });
        ...
        // Determinar si es CI y configurar inicialmente
        @if(old('documento_id'))
            esCI = ("{{ old('documento_id') }}" == '1');
        @else
            esCI = ("{{ $proveedore->persona->documento_id }}" == '1');
        @endif

        // Configurar campos de documento según si es CI
        if (esCI) {
            $('#box-complemento').show();
            $('#box-numero-documento').removeClass('col-md-6').addClass('col-md-3');
            dividirNumeroDocumentoCI();
